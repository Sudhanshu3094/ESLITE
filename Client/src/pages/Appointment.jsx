    ...
    <div>
      ...
      <div className='sm:ml-72 sm:pl-4 mt-4 font-medium text-gray-700'>
        ...
        <div className='flex gap-3 items-center w-full overflow-x-scroll mt-4'>
          ...
          <div onClick={() => setSlotIndex(index)} className={`text-center py-6 min-w-16 rounded-full cursor-pointer ${slotIndex === index ? 'bg-neutral-300 text-zinc-500' : 'border border-gray-200'}`} key={index}>
        ...
        {engSlots.length && engSlots[slotIndex].map((item, index) => (
          <p onClick={()=>setSlotTime(item.time)} className={`text-sm font-light flex-shrink-0 px-5 py-2 rounded-full cursor-pointer ${item.time === slotTime? 'bg-neutral-300 text-gray-500' : 'text-gray-400 border border-gray-300'}`} key={index}>{item.time.toLowerCase()}</p>
        ))}
      </div>
      <button className='bg-neutral-300 text-gray-500 text-sm font-light px-14 py-3 rounded-full my-6'>Book an appointment</button>
      </div>
